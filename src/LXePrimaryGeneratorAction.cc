...
void LXePrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  else if ( fGenerator == "double" ) Generate_doubleelec(anEvent);
  else if ( fGenerator == "triple" ) Generate_tripleelec(anEvent);
...
void LXePrimaryGeneratorAction::Generate_singleelec(G4Event* anEvent)
{
    ...
    G4ThreeVector decayVertex;
    GetUnifRandPosInLXe(decayVertex);
    fParticleGun3->SetParticlePosition(decayVertex);
    ...
    fParticleGun3->SetParticleEnergy(fQ_value);
    fParticleGun3->SetParticleMomentumDirection(G4ThreeVector(ux1,uy1,uz1));
    fParticleGun3->GeneratePrimaryVertex(anEvent);
}

void LXePrimaryGeneratorAction::Generate_doubleelec(G4Event* anEvent)
{
    ...
    fParticleGun3->SetParticleEnergy(fQ_value/2.);
    fParticleGun3->SetParticleMomentumDirection(G4ThreeVector(ux1,uy1,uz1));
    fParticleGun3->GeneratePrimaryVertex(anEvent);
...
void LXePrimaryGeneratorAction::Generate_tripleelec(G4Event* anEvent)
{
    ...
    fParticleGun3->SetParticleEnergy(fQ_value/3.);
    fParticleGun3->SetParticleMomentumDirection(G4ThreeVector(ux1,uy1,uz1));
    fParticleGun3->GeneratePrimaryVertex(anEvent);
    ...
    theta = acos(2*(G4UniformRand()-0.5));
    phi   = 2*pi*G4UniformRand();
    ux1   = sin(theta)*cos(phi);
    uy1   = sin(theta)*sin(phi);
    uz1   = cos(theta);
