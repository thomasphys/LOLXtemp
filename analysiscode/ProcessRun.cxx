...
int main(int argc, char** argv)
{
    ...
    std::vector<double> neighbourcount(n_sipm,0.0);
    ...
    for(int i=0; i<n_sipm; ++i){
       ...
       for(int j=1; j<5; ++j){
           if(hasfilter[neighbour[i][j].first]){
                   neighbourcount[i] += 1.0;
           }
       }
    }
    ...
    TH2F* Filtered_NonFiltered_postcut = new TH2F("Filtered_NonFiltered_postcut","",10000,0.,10000.,10000,0,10000);
    TH2F *fMaxHist_postcut = new TH2F("fMax_postcut","",10000,0.,10000.,100,0.,1.0);
    TH2F *GroupfMaxHist_postcut = new TH2F("GroupfMax_postcut","",10000,0.,10000.,100,0.,1.0);
    ...
    TH1F *Waveform_unfiltered = new TH1F("Waveform_unfiltered","",1000,-0.5,999.5);
    TH1F *Waveform_filtered = new TH1F("Waveform_filtered","",1000,-0.5,999.5);
    ...
    for(int i=0; i<nentries; ++i){
        ...
        for(int i_mppc=0; i_mppc<n_mppc; ++i_mppc){
            ...
            int nphotons = mppc->GetHitCount();
            fMax[siPMid] += nphotons;
            if(hasfilter[siPMid]){
                integral_filtered += nphotons;
                for(int jj=0; jj<nphotons; ++jj){
                    Waveform_filtered->Fill(mppc->GetHitTime(jj));
                }
                ...
                integral_nonfiltered += nphotons;
                for(int jj=0; jj<nphotons; ++jj){
                    Waveform_unfiltered->Fill(mppc->GetHitTime(jj));
                }
            }
        ...
        for(int jj=0; jj<n_sipm; ++jj){
            ...
            groupfMax[jj] /= neighbourcount[jj];
            fMax[jj] /= integral_filtered;
            if(groupfMax[jj]>max_groupfmax){
                max_groupfmax = groupfMax[jj];
                jj_groupfmax = jj;
            }
            if(fMax[jj]>max_fmax){
                max_fmax = fMax[jj];
    ...
    if(hasfilter[jj_groupfmax]){
        Filtered_NonFiltered_postcut->Fill(integral_nonfiltered, integral_filtered);
        fMaxHist_postcut->Fill(integral_filtered,max_fmax);
        GroupfMaxHist_postcut->Fill(integral_filtered,max_groupfmax);
    }
    ...
    double cherfactor = 0.0;
    double chereff = 0.0;
    for(int jj=0; jj<nsamples; ++jj){
        //cherfactor += 1.0/nsamples;
        cherfactor += LOLXReadData::GetSiPM_Efficiency(ds->GetCherenkovWavelength(jj))*LOLXReadData::GetFilter_Trans(ds->GetCherenkovWavelength(jj));
        chereff += LOLXReadData::GetSiPM_Efficiency(ds->GetCherenkovWavelength(jj));
    }
    cherfactor /= nsamples;
    chereff /= nsamples;
        
    double vuveff = LOLXReadData::GetSiPM_Efficiency(1240./175.);

        //cherfactor = 1.0;
        
	MCScintillation_Vs_MCCherenkov->Fill(fillfactor*(vuveff*ds->GetMCSintillation()*(1.0/(n_sipm))+chereff*ds->GetMCCherenkov()*(1.0/(n_sipm))),
					     cherfactor*fillfactor*(ds->GetMCCherenkov()*(((double)(n_sipm-1))/(n_sipm))));
	
	for(int jj=0; jj<nsamples; ++jj){
		NMCCherenkov_Vs_CherSpectrum->Fill(ds->GetMCCherenkov(),1240./ds->GetCherenkovWavelength(jj));
    ...
    fMaxHist->Write();
    GroupfMaxHist->Write();
    Filtered_NonFiltered_postcut->Write();
    fMaxHist_postcut->Write();
    GroupfMaxHist_postcut->Write();
    ...
    Waveform_unfiltered->Write();
    Waveform_filtered->Write();
    fout->Close();
